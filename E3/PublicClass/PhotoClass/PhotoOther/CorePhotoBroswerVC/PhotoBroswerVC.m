...
@property (weak, nonatomic) IBOutlet PBSaveBtn *saveBtn;

@property (nonatomic,assign) void(^dissBlock)(NSInteger currIndex);
...
+(void)show:(UIViewController *)handleVC superView:(UIScrollView *)scrollview type:(PhotoBroswerVCType)type index:(NSUInteger)index photoModelBlock:(NSArray *(^)())photoModelBlock{
    ...
    pbVC.superScrollview = scrollview;
...
- (IBAction)leftBtnClick:(id)sender {
    ...
    [self dismiss];
//    self.dissBlock(self.page);
    CGPoint point = CGPointMake(CGRectGetWidth(self.superScrollview.frame) * self.page, 0);
    [self.superScrollview setContentOffset:point];
}
